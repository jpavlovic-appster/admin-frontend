...
export class StartAnimationComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,
    ...
    this.configForm = this.formBuilder.group({
      ...
      startAnimJsonFile: ['', [Validators.required]],
      startAnimPngFile: ['', [Validators.required]],
    });
  ...
  selectFile(files: any) {
    ...
    if (extension != 'json') {
      ...
      this.configForm.controls['startAnimJsonFile'].reset()
        return;
    }
  ...
  selectFile2(files: any) {
    ...
    if (extension != 'png') {
      ...
      this.configForm.controls['startAnimPngFile'].reset()
        return;
    }
  ...
  onSubmit() {
    ...
    if (this.selectLogo && this.selectLogo.name) {
      fd.append('startAnimJsonFile', this.selectLogo, this.selectLogo.name);
    }

    if (this.selectLogo2 && this.selectLogo2.name) {
      fd.append('startAnimPngFile', this.selectLogo2, this.selectLogo2.name);
    }
